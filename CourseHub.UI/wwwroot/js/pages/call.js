...
function setupRTCHandler(rtcHandler) {
    ...
    rtcHandler.onRoomInfoReceived = async (roomInfo) => {
        if (roomInfo.id == _roomId) {
            ...
            await setupConnections(peerIds);
            addRoomUI(peers);
            await tryGetMedia(_defaultMedia);
    ...
    rtcHandler.onOfferReceived = async (peer, offer, isRenegotiation) => {
        // doesn't override existing connections & behaviors (if there are)
        if (!isRenegotiation) {
            await setupConnections([peer]);
            if (_localStream != null)
    ...
    async function setupConnections(peers) {
        await rtcHandler.setupRTCPeerConnections(peers);
